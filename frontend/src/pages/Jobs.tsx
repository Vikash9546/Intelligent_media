...
const Jobs: React.FC = () => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'failed': return 'audio_file'; // just mapping icons for visuals
      case 'processing': return 'movie';
      case 'completed': return 'image';
      default: return 'video_file';
    }
  };
                ...
                <td className="px-lg py-md">
                  <div className="flex flex-col items-center gap-1">
                    ...
                    {job.status === 'processing' && (
                      <div className="w-24 h-1 bg-surface-container-highest rounded-full overflow-hidden mt-1">
                        <div className="h-full progress-gradient w-[65%]"></div>
                      </div>
                    )}
                  </div>
                </td>
                  ...
                  {job.status === 'completed' ? (
                    <div className="flex items-center gap-2">
                      <div className="flex items-center gap-1">
                        ...
                        {job.qualityScore > 90 && (
                          <span className="material-symbols-outlined text-[16px] text-emerald-500" data-icon="verified">verified</span>
                        )}
                      </div>
                    </div>
                  ) : job.status === 'failed' ? (
                    <span className="text-on-surface-variant italic opacity-50">N/A</span>
                  ) : (
                    <span className="font-code-sm text-code-sm">--</span>
                  )}
                </td>
