...
const Dashboard = () => {
  ...
  return <div className="max-w-[1400px] mx-auto grid grid-cols-12 gap-gutter">
      {
    /* Welcome Header */
  }
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-outline-variant">
                {recentJobs.map((job) => <tr key={job.id} className="hover:bg-surface-container-low/30 transition-colors">
                    ...
                    <td className="px-lg py-md">
                      <div className={`flex items-center gap-xs font-label-md ${job.status === "completed" ? "text-emerald-600" : job.status === "failed" ? "text-error" : "text-secondary"}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${job.status === "completed" ? "bg-emerald-600" : job.status === "failed" ? "bg-error" : "bg-secondary animate-pulse"}`} />
                        {job.status.charAt(0).toUpperCase() + job.status.slice(1)}
                      </div>
                    </td>
                    <td className="px-lg py-md">
                      {job.status === "processing" ? <div className="w-full bg-surface-container-high h-1.5 rounded-full overflow-hidden">
                          <div className="progress-gradient h-full w-[45%]" />
                        </div> : <p className="font-label-md">{job.status === "completed" ? "100%" : "0%"}</p>}
                    </td>
                    <td className="px-lg py-md">
    ...
    /* Quick Upload CTA */
        ...
        <div className="bg-[#2e1065] rounded-xl p-lg text-white shadow-xl relative overflow-hidden group">
          <div className="absolute inset-0 opacity-10 bg-[radial-gradient(circle_at_50%_50%,rgba(255,255,255,0.4),transparent)] transition-transform group-hover:scale-150" />
    ...
    /* Processing Activity Feed */
        ...
        <div className="glass-card rounded-xl flex flex-col flex-grow">
          ...
          <div className="p-lg space-y-lg overflow-y-auto max-h-[400px]">
            {recentJobs.map((job, idx) => <div key={job.id} className="flex gap-md relative">
                {idx < recentJobs.length - 1 && <div className="absolute left-2.5 top-8 bottom-0 w-[1px] bg-outline-variant" />}
                <div className={`relative z-10 w-5 h-5 rounded-full flex items-center justify-center border-2 border-white ${job.status === "completed" ? "bg-emerald-100" : job.status === "failed" ? "bg-red-100" : "bg-secondary-fixed"}`}>
                  <span className={`material-symbols-outlined text-[12px] ${job.status === "completed" ? "text-emerald-600" : job.status === "failed" ? "text-error" : "text-secondary animate-spin"}`} data-icon={job.status === "completed" ? "check" : job.status === "failed" ? "close" : "autorenew"}>
                    {job.status === "completed" ? "check" : job.status === "failed" ? "close" : "autorenew"}
                  </span>
                </div>
                <div className="pb-lg">
                  <p className="font-body-md">
                    <span className="font-bold">{job.originalFilename}</span> {job.status === "completed" ? "processing finished." : job.status === "failed" ? "processing failed." : "being analyzed..."}
                  </p>
            ...
            {recentJobs.length === 0 && <p className="text-center text-outline font-body-md py-lg">No activity yet.</p>}
          </div>
        </div>
      </div>
    </div>;
